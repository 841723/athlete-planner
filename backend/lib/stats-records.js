...
export function buildStatsRecords(completed) {
  ...
  const records = [
    buildRecordCard("highest_avg_hr", "heart-crack", "FC media más alta", highestAvgHr, "hr"),
    buildRecordCard("highest_max_hr", "heart-pulse", "FC máx más alta", highestMaxHr, "hr"),
    buildRecordCard("lowest_avg_hr", "heart", "FC media más baja", lowestAvgHr, "hr"),
    buildRecordCard("lowest_max_hr", "heart-off", "FC máx más baja", lowestMaxHr, "hr"),
    buildRecordCard("hottest", "thermometer", "Más calor", hottest, "temp"),
    buildRecordCard("coldest", "snowflake", "Más frío", coldest, "temp"),
    buildRecordCard("longest", "clock", "Sesión más larga",
      longest ? { value: getSessionTime(longest.session), session: longest.session } : null, "duration"),
    buildRecordCard("shortest", "zap", "Sesión más corta",
      shortest ? { value: getSessionTime(shortest.session), session: shortest.session } : null, "duration"),
    buildRecordCard("earliest", "sunrise", "Más temprano", earliestVal, "hour"),
    buildRecordCard("latest", "moon", "Más tarde", latestVal, "hour"),
    {
      id: "two_day_sessions",
      icon: "calendar-clock",
    ...
    buildRecordCard("most_calories", "flame", "Más calorías", mostCalories, "calories"),
    buildRecordCard("most_elevation", "mountain", "Más desnivel", mostElevation, "elevation"),
    buildRecordCard("longest_distance", "ruler", "Distancia más larga", longestDistance, "distance"),
    buildRecordCard("highest_watts", "zap", "Potencia más alta", highestWatts, "watts"),
    buildRecordCard("highest_te", "trending-up", "Mayor Training Effect", highestTe, "te"),
    {
