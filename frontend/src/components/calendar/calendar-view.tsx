      ...
      <div className='animate-fade-in'>
          ...
          <div className='grid grid-cols-[repeat(7,minmax(0,1fr))] lg:grid-cols-[2.5rem_repeat(7,minmax(0,1fr))] gap-0.5 sm:gap-1'>
              ...
              {["Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"].map((day) => (
                  <div
                      key={day}
                      className='text-center text-[10px] sm:text-xs font-medium text-gray-500 py-1.5 sm:py-2'
                  >
                      {day}
                  </div>
              ))}
              {days.map((day, i) => {
                  ...
                  return (
                      <div key={dateStr} className='contents'>
                          {isWeekStart && (
                              <div
                                  className={`hidden lg:flex items-center justify-center text-xs font-semibold select-none ${isOtherMonth ? "text-gray-600" : "text-gray-500"}`}
                              >
                                  W
                                  {getWeekNumber(
                                      day,
                                      meta?.trainingWeekOneStart ??
                                          "2026-05-11",
                                  )}
                              </div>
                          )}
                              ...
                              return (
                                  <button
                                      ...
                                      className={`w-full flex items-center gap-3 p-3 rounded-xl text-left transition-colors ${
                                          isPlanned
                                              ? "border border-dashed border-white/20 bg-dark-300/30"
                                  ...
                                  >
                                      ...
                                      <div className='flex-1 min-w-0'>
                                          ...
                                          <p className='text-xs text-gray-500'>
                                              {getSportLabel(s.category)}
                                          </p>
                                      </div>
                                      {s.start_date_local && (
                                          <span className='text-xs text-gray-400 flex-shrink-0'>
                                              {format(
                                                  parseISO(s.start_date_local),
                                                  "HH:mm",
                                              )}
                                          </span>
                                      )}
          ...
          {selectedSession && selectedSession.status === "planned" && (
              ...
              >
                  <div
                      className='card p-6 max-w-md w-full max-h-[80vh] overflow-y-auto animate-scale-in'
                      onClick={(e) => e.stopPropagation()}
                  >
                      <div className='flex items-center justify-between mb-4'>
                      ...
                      <div className='space-y-3 text-sm'>
                          <div className='flex justify-between'>
                              ...
                              <span>
                                  {formatFullDate(
                                      selectedSession.start_date_local,
                                  )}
                              </span>
                          </div>
                          <div className='flex justify-between'>
                              ...
                              <span>
                                  {getSportLabel(selectedSession.category)}
                              </span>
                          </div>
                          {selectedSession.workout_text ? (
                              <div className='pt-2 border-t border-dark-400'>
                                  ...
                                  <div className='mt-2'>
                                      <WorkoutText
                                          text={selectedSession.workout_text}
                                      />
                                  </div>
                              </div>
                          ) : (
                              ...
                              <div className='flex gap-2 pt-3 border-t border-dark-400'>
                                  ...
                                  <button
                                      ...
                                      onClick={() => {
                                          if (
                                              window.confirm(
                                                  `¿Eliminar "${selectedSession.title ?? selectedSession.name}"?`,
                                              )
                                          ) {
          ...
          <PlannedFormModal
              open={formOpen}
              session={
                  selectedSession?.status === "planned" ? selectedSession : null
              }
