...
export function WeeklyVolume() {
  ...
  const maxSessions = Math.max(...sports.map((k) => bySport[k]), 1);
  const totalSessions = weekCompleted.length;

  return (
    <div className="card p-5 animate-slide-up">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-lg font-semibold">Volumen semanal</h2>
        <span className="text-xs text-gray-400">{totalSessions} {totalSessions === 1 ? "sesión" : "sesiones"}</span>
      </div>
      <div className="space-y-3">
        {sports.map((sport) => {
          ...
          const count = bySport[sport];
          return (
            <div key={sport}>
              <div className="flex items-center justify-between text-xs mb-1">
                <span className="text-gray-300">{label}</span>
                <span className="text-gray-400">{count} {count === 1 ? "sesión" : "sesiones"}</span>
              </div>
              <div className="h-2 rounded-full bg-dark-400 overflow-hidden">
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{ width: `${(count / maxSessions) * 100}%`, backgroundColor: color }}
                />
              </div>
            </div>
          );
